...
# Suprimir avisos não críticos do scikit-image e scipy durante o processamento
warnings.filterwarnings('ignore')

# Verificar se o script de visualização do pipeline existe no disco;
# é lançado como subprocess separado para evitar conflito entre TkAgg e threads
_PIPELINE_SCRIPT = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                'auxiliary_codes', 'passo7_pipeline_viz.py')
_PIPELINE_OK = os.path.exists(_PIPELINE_SCRIPT)

# Verificar se o script Canny interactivo existe; abre uma janela separada que
# demonstra como as vagas podem ser detectadas automaticamente numa imagem vazia
_CANNY_SCRIPT = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                              'auxiliary_codes', 'passo_teste_canny_interativo.py')
_CANNY_OK = os.path.exists(_CANNY_SCRIPT)

# ── Caminhos ────────────────────────────────────────────────────────────────────
# Resolver caminhos absolutos para o dataset, limiares calibrados e vagas extra;
# tudo o que a aplicacao precisa está dentro de auxiliary_codes/
_SCRIPT_DIR      = os.path.dirname(os.path.abspath(__file__))
AUX_DIR          = os.path.join(_SCRIPT_DIR, 'auxiliary_codes')
DATASET          = os.path.normpath(os.path.join(_SCRIPT_DIR, '..', '..', 'dataset_parkinglot'))
DATASET_V2       = os.path.join(AUX_DIR, 'dataset_parkinglot_v2')
CALIB_FILE       = os.path.join(AUX_DIR, 'passo3_limiares.json')
EXTRA_VAGAS_FILE = os.path.join(AUX_DIR, 'vagas_indefinidas', 'vagas_extra_g100_crop.json')

# Identificadores de categoria conforme as anotações COCO: 1=livre, 2=ocupada
CAT_LIVRE   = 1
CAT_OCUPADA = 2

# ── Tema de cores (Catppuccin Mocha) ────────────────────────────────────────────
# Constantes de cor usadas em toda a interface: fundos, texto, acentos e estados
BG        = '#1e1e2e'
BG_PANEL  = '#181825'
BG_CANVAS = '#11111b'
BG_TB     = '#12121c'
...
# Cores RGBA do preenchimento dos rectângulos sobrepostos na imagem (overlay PIL):
# apenas 2 estados visuais — verde=livre, vermelho=ocupada — independente do acerto
COL_LIVRE = (  0, 210,  80, 110)
COL_OCUP  = (220,  40,  40, 120)

# Cores RGB das bordas dos rectângulos (contorno mais fino por cima do preenchimento)
BRD_LIVRE = (  0, 230,  80)
BRD_OCUP  = (230,  40,  40)


# ── Pipelines e features (identicos ao passo4_avaliacao_final.py) ───────────────
# Os mesmos algoritmos do passo3/passo4 são reutilizados aqui sem alteração,
# garantindo que a app demo classifica exactamente como o avaliador em batch.

# Função auxiliar de nitidez: aplica unsharp mask e garante valores no intervalo [0, 1]
def _unsharp(g):
    return np.clip(unsharp_mask(g, radius=2, amount=1.0), 0.0, 1.0)

# Dicionário de pipelines disponíveis: cada entrada é uma função que transforma
# a imagem cinzenta completa com uma sequência de filtros (gaussiano, mediana, unsharp)
PIPELINE_FUNCS = {
    'raw'          : lambda g: g,
...
# Configuração específica de cada parque: quais 3 features usar (pipeline + nome da feature + rótulo),
# parâmetros do detector de cantos Harris (F4) e cor de identificação na barra de título
PARK_CFG = {
    'G28': {
        'n_vagas'    : 28,
...
# Tabela de consulta inversa: nº de vagas anotadas → nome do parque
# (permite identificar automaticamente o parque a partir de qualquer imagem do dataset)
N_VAGAS_TO_PARQUE = {v['n_vagas']: p for p, v in PARK_CFG.items()}


# Calcular uma propriedade da GLCM (Gray-Level Co-occurrence Matrix) da ROI:
# quantiza a 64 níveis, computa a matriz de co-ocorrência e extrai a propriedade pedida
def _glcm(roi, prop):
...
# Dicionário de funções de extracção de features: cada função recebe a ROI cinzenta
# já filtrada e devolve um único valor escalar que caracteriza o conteúdo da vaga
FEAT_FUNCS = {
...
# Feature F4: contar cantos de Harris na ROI usando os parâmetros calibrados por parque
# (threshold de resposta e distância mínima entre cantos); devolve 0.0 se falhar
...
# Passo 1 do motor: pré-processar a imagem inteira com todos os pipelines necessários
# para o parque em causa — aplica os filtros uma vez ao frame completo, não vaga a vaga
...
# Passo 2 do motor: para cada vaga, recortar a sua ROI das imagens já pré-processadas
# e calcular os 4 valores de feature (f1, f2, f3 via FEAT_FUNCS + f4 via Harris)
...
# Passo 3 do motor: classificar a vaga por votação maioritária — cada feature ativa
# compara o seu valor com o limiar calibrado e vota OCUPADA ou LIVRE; ganha a maioria
...
class ParqueApp:
    ...
    def _load_coco_data(self):
        ...
        # 2) Dataset unificado (v2) -- sobrescreve img_dir para todas as imagens presentes
        #    G100 usa coordenadas ajustadas (crop 575x371); G28/G40 usam coords originais (640x640)
        if os.path.isdir(DATASET_V2):
            for split in ('train', 'valid', 'test'):
                path = os.path.join(DATASET_V2, split, '_annotations.coco.json')
                if not os.path.exists(path):
                    continue
                with open(path, encoding='utf-8') as f:
                    data = json.load(f)
                ...
                for img in data['images']:
                    ...
                    self.coco_lut[fn] = {
                        'split'  : split,
                        'anns'   : anns,
                        'img_dir': os.path.join(DATASET_V2, split),
                    }
    ...
    def abrir_imagem(self):
        # Preferir o dataset v2 (em auxiliary_codes), senão o dataset original
        init_dir = os.path.join(DATASET_V2, 'test')
